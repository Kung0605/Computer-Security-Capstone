...
int main() {
    execCommand("echo 0 | sudo tee /proc/sys/net/ipv4/ip_forward");
    execCommand("echo 1 | sudo tee /proc/sys/net/ipv4/conf/*/send_redirects");
    ...
    system(("sudo ethtool -K" + interface + "gro off >/dev/null 2>&1").c_str());
